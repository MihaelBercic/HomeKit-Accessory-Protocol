...
class AutoMower(aid: Long, name: String, ip: String, private val additionalData: String) : Accessory(aid, name) {
    ...
    private fun getMowerDetails(): AutoMowerDataResponse {
        val request = HttpRequest.newBuilder(URI(mowerEndpoint))
            .GET()
            .apply(this::authenticationHeaders)
            .build()
        val response = httpClient.send(request, BodyHandlers.ofString())
        return gson.fromJson(response.body(), AutoMowerDataResponse::class.java).apply(Logger::info)
    }

    override fun setup(configurationDetails: Map<String, Any>, bridgeAddress: String) {
        Logger.info(mowerData)
        ...
        addService(3, ServiceType.Fan) {
            val activeCharacteristic = add(CharacteristicType.Active) {
                if (value != null && System.currentTimeMillis() - lastUpdate >= 10000) {
    ...
    override fun commitChanges() {
        ...
        Logger.info("Committing changes: $nextRequest")
        val request = HttpRequest.newBuilder(URI("$mowerEndpoint/actions".apply(Logger::info)))
            .POST(BodyPublishers.ofString(gson.toJson(nextRequest)))
            .apply(this::authenticationHeaders)
            .build()
        val response = httpClient.send(request, BodyHandlers.ofString())
        nextRequest = null
